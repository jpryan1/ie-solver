...
double Kernel::stokes_kernel(unsigned int i, unsigned int j) const {
	...
	// if(i%2==0 && j%2==0){
	// 	return boundary->weights[dof_j]*scale*(log(1.0/r.norm()) + (r0*r0/r.dot(r)));
	// }
	// else if(i%2==1 && j%2==1){
	// 	return boundary->weights[dof_j]*scale*(log(1.0/r.norm()) + (r1*r1/r.dot(r)));
	// }
	// else{
	...
	if(dof_i==dof_j){
		...
		if(i%2==0 && j%2==0){
			return -0.5 - 
			0.5*boundary->curvatures[dof_i]*boundary->weights[dof_i]*scale*t0*t0;
		}
		else if(i%2==1 && j%2==1){
			return -0.5 - 
			0.5*boundary->curvatures[dof_i]*boundary->weights[dof_i]*scale*t1*t1;
		}
		else{
			return -0.5 * boundary->curvatures[dof_i]*boundary->weights[dof_i]*scale*t0*t1;
		}
	}
	...
	double potential = boundary->weights[dof_j]*scale*(r.dot(n))/(pow(r.dot(r),2));
