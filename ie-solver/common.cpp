...
int ie_Mat::id( std::vector<int>& p, ie_Mat& Z, double tol){
		...
		// go down diagonal until satisfied with error
		// set p to be [sk rd]
		if(skel == -1){
		...
		// set Z to be R_11^-1 R_12
		int redund = width_ - skel;
		
	

		LAPACKE_dtrtrs(CblasColMajor, 'U', 'N', 'N', skel, redund, cpy.mat, cpy.lda_, cpy.mat+cpy.lda_*skel, cpy.lda_);
