...
//   //top right flat
//   // NOTE THE <=flat_points, this means we are now adding the top right
//   // corner point!
//   for (int i = 0; i <= flat_points; i++) {
...
//   for (int i = 1; i < flat_points; i++) weights.push_back(0.25 /
//   flat_points);


//   //top sine

//   weights.push_back((E[sine_slice] - E[sine_slice - 1] + (0.25 /
// flat_points)) /
//                     2.0);
...
//   // Now we have all the weights we need, for simplicity, we now enter the
//   // rest of the
...
//   int w_ind = i/2;
//   double dist1 = sqrt(pow(points[i] - points[i-2],2) + pow(points[i+1]-
// points[i-1],2));
//   double dist2 = sqrt(pow(points[i] - points[i+2],2) + pow(points[i+1]-
// points[i+3],2));
